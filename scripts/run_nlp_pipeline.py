...
PIPELINE_STAGES: List[PipelineStage] = [
    PipelineStage(
        ...
        rationale="Use existing raw snapshots to produce staged consolidated comments.",
    ),
    PipelineStage(
        key="text_preprocessing",
        title="Text Preprocessing",
        kind="script",
        target="scripts/run_moltbook_text_preprocessing.py",
        rationale="Normalize and clean staged comments into a dedicated preprocessed dataset.",
    ),
    PipelineStage(
        key="eda",
        title="EDA",
        kind="script",
        target="scripts/run_moltbook_eda_stage.py",
        rationale="Produce exploratory summaries focused on the rule-based pipeline inputs.",
    ),
    PipelineStage(
        key="feature_extraction",
        title="Feature Extraction",
        kind="script",
        target="scripts/run_moltbook_feature_extraction.py",
        rationale="Extract interpretable lexical and structural text features for descriptive analysis.",
    ),
    PipelineStage(
        key="rule_based_tools",
        title="Three Rule-Based Sentiment Tools",
        kind="script",
        target="scripts/run_moltbook_rule_based.py",
        rationale="Apply VADER, SentiWordNet, and ensemble rule-based sentiment scoring.",
    ),
    PipelineStage(
        key="interaction_network",
        title="RQ1 Interaction Network (separate)",
        kind="script",
